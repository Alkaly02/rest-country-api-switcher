    ...
    <div style={{zIndex: '999', left: '45%'}} className='bg-light p-2 position-absolute text-dark'>
        {
            ...
            .map((country, index) => (
                <div style={{cursor: 'pointer'}} className='my-1' key={`${country.name.common}-${index}`}>
                    <span onClick={() => searchCountryOnAutoComplete(country.name.common) }> <img style={{width: '20px', fontSize: '0.8rem'}} src={country.flags.png} alt='Crountry flag' /> {country.name.common}</span>
                </div> 
            ))
        }
    </div>
